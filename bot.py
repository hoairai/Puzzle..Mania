...
class PuzzleMania:
    ...
    def generate_payload(self, account: str, address: str, nonce: str):
        try:
            message = f"puzzlemania.0g.ai wants you to sign in with your Ethereum account:\n{address}\n\nBy signing, you are proving you own this wallet and logging in. This does not initiate a transaction or cost any fees.\n\nURI: https://puzzlemania.0g.ai\nVersion: 1\nChain ID: 56\nNonce: {nonce}\nIssued At: 2025-04-15T17:35:35.515Z\nResources:\n- https://privy.io"
    ...
    async def user_init(self, address: str, proxy=None, retries=5):
    ...
    async def user_authenticate(self, account: str, address: str, nonce: str, proxy=None, retries=5):
    ...
    async def user_login(self, auth_token: str, proxy=None, retries=5):
        data = json.dumps({
            "operationName":"UserLogin",
            "variables":{
                "data":{
                    "externalAuthToken":auth_token
                }
            },
    ...
    async def user_data(self, access_token: str, proxy=None, retries=5):
        data = json.dumps({
    ...
    async def task_lists(self, access_token: str, proxy=None, retries=5):
        data = json.dumps({
    ...
    async def complete_task(self, identity_token: str, access_token: str, task_id:str, proxy=None, retries=5):
        data = json.dumps({
    ...
    async def process_get_token(self, account: str, address: str, use_proxy: bool):
        ...
        if nonce:
            ...
            while token is None:
                ...
                return identity_token, auth_token
    ...
    async def process_get_access_token(self,account: str, address: str, use_proxy: bool):
        ...
        if identity_token and auth_token:
            proxy = self.get_next_proxy_for_account(address) if use_proxy else None
                ...
                if tasks:
                    for task in tasks:
                        if task:
                            ...
                            if complete and complete.get("data", None):
                                self.log(
